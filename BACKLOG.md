...
| 4 | BE | 복수 좌석의 입력 순서가 달라 deadlock이 발생할 수 있다 | 반대 순서 fixture, index 전후 query plan·첫 lock·DB 예외와 rollback | 무제한 재시도 | lock ordering·migration 중복 기준선 완료 ([#9](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/9), [#11](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/11), [#15](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/15), [#17](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/17)); Entity 신규 schema 제약·V1 Flyway baseline 및 fresh/baseline history contract 완료 ([#59](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/59), [#152](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/152), [PR #153](https://github.com/SKUWooU/TicketOnBoarding_Be/pull/153)); 실제 legacy DB upgrade·data migration은 별도 보류 |
...
| 8-INSTANCE | BE | 단일 instance에서 검증한 JWT·hold·Checkout 전이가 요청 분산 경계에서도 수렴하는지 확인되지 않았다 | shared JWT key, loopback proxy, instance별 metric, 2,000석 fixture final snapshot | 운영 LB·Kafka·outbox·Redis lock 선제 도입 | 완료 ([#150](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/150), [PR #151](https://github.com/SKUWooU/TicketOnBoarding_Be/pull/151)); local two-instance smoke로 JWT 공유·종료 불변식만 확인했으며 운영 확장성 결론은 보류 |
| 9 | BE | 누적된 로컬 고경합 결과를 분석할 때 반복 수작업과 원시 로그 노출 위험이 있다 | 읽기 전용 MCP로 allowlist summary·도메인 불변식만 제공 | 운영 DB·외부 API·부하 실행 도구 | 서버·로컬 Codex 등록·조회 계약 완료 ([#146](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/146), [#148](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/148)); run 비교·실행 도구는 승인·권한 경계·반복 분석 수요가 확인된 뒤 별도 검토 |
