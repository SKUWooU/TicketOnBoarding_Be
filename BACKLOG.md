...
| 9 | BE | 누적된 로컬 고경합 결과를 분석할 때 반복 수작업과 원시 로그 노출 위험이 있다 | 읽기 전용 MCP로 allowlist summary·도메인 불변식만 제공 | 운영 DB·외부 API·부하 실행 도구 | 1차 완료 ([#146](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/146)); run 비교·실행 도구는 승인·권한 경계·반복 분석 수요가 확인된 뒤 별도 검토 |
