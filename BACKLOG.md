...
| 5 | BE/FE | 예약과 결제 완료가 결합되고 상태 전이가 불명확하다 | 현재 흐름 재현, mock 결제, 허용·거부 전이 테스트 | 실제 결제 실행 | 서버 미검증 결제 완료 기준선 완료 ([#37](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/37)); 예약 상태 enum·전이 정책 구현·로컬 검증 완료 ([#41](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/41)); 서버 가격·Payment/mock 검증 후속 |
