...
| 5-HOLD | BE/FE | 좌석 선택과 검증된 예약 사이에 서버 임시 점유·소유자·만료가 없어 같은 가용 좌석을 여러 사용자가 선택할 수 있다 | 반복 snapshot, 독립 사용자 동시 경쟁, 점유 소유권·만료·회수 fixture | Redis·분산 lock·scheduler 선제 도입 | 부재 기준선 진행 중 ([#61](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/61)); 상태·소유권·만료 정책 구현은 후속 Issue |
