...
| 5 | BE/FE | 예약과 결제 완료가 결합되고 상태 전이가 불명확하다 | 현재 흐름 재현, mock 결제, 허용·거부 전이 테스트 | 실제 결제 실행 | 서버 미검증 기준선 ([#37](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/37)), 예약 상태 enum·전이 ([#41](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/41)), 서버 가상 가격·Payment/mock 검증 경계 ([#43](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/43)) 구현; Frontend 전환·legacy 제거 후속 |
...
| 7 | BE | 고경합 병목 위치가 측정되지 않았다 | 2,000석 fixture·k6·Actuator 실행 기반 구성 후 단계별 TPS·p95·오류율, lock wait, Hikari 시계열 측정 | 운영 SLA 주장 | 측정 기반 ([#47](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/47)), 경합 HTTP 계약 ([#49](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/49), [PR #50](https://github.com/SKUWooU/TicketOnBoarding_Be/pull/50)), 경량 관측 경계 ([#51](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/51)) 완료; 단계별 반복 기준선에서 distributed 50→100 RPS 변곡 확인 ([#53](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/53)); SQL별 진단에서 좌석 잠금 SELECT를 병목 후보로 격리 ([#55](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/55)); 고정 2,000행 복합 unique index A/B에서 100 RPS p95 3,064.39→143.41ms·pending 189→0 확인 ([#57](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/57)); Entity·migration 경계의 영구 적용 후속 |
