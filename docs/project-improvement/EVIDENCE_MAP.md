...
| 실제 Backend 로컬 실행 경로 부재 | 빈 `application.yml`, `bootRun` property 부재 | MariaDB Compose와 공개 가능한 local profile | Compose config·health, `bootRun` startup, `GET /main`, DB row·SQL 로그 | MariaDB healthy, port 18080 기동, HTTP 200 JSON, admin 1·concert 0과 응답 일치 | [BE #19](https://github.com/SKUWooU/TicketOnBoarding_Be/issues/19) |
