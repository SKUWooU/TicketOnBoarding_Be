...
최초 distributed 5 RPS·10초 smoke에서는 예약 51건, 비-2xx 0건, HTTP p95 188.32ms였고 종료 불변식을 충족했다. 이는 스크립트·fixture·관측 경로가 함께 동작한다는 실행 증거이지 안정 처리량이나 개선 전후 수치가 아니다. 종료 뒤 Hikari와 MariaDB 상태도 peak가 아니므로 시계열 관측 없이는 병목 위치를 단정할 수 없다.
