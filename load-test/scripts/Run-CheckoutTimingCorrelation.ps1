...
function Restore-CheckoutBaselineMariaDb {
    & docker compose -f (Join-Path $root 'compose.yml') up -d --force-recreate mariadb
    if ($LASTEXITCODE -ne 0) { throw 'Failed to restore baseline MariaDB.' }
    $restoreDeadline=(Get-Date).AddSeconds(60)
    do { try { if ((Invoke-RestMethod -Uri "$ManagementBaseUrl/actuator/health" -TimeoutSec 2).status -eq 'UP') { return } } catch {}; Start-Sleep -Seconds 2 } while ((Get-Date) -lt $restoreDeadline)
    throw 'Backend did not recover after baseline MariaDB restore.'
}
trap { Restore-CheckoutBaselineMariaDb; throw $_ }
