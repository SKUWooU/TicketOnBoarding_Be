...
- PR: [#149](https://github.com/SKUWooU/TicketOnBoarding_Be/pull/149) / squash `5ceab0e`
...
- 제외: k6/Docker 실행, DB 변경, KOPIS·PG·OAuth 호출, 원시 로그·CSV 노출, 운영 성능 주장
...
- 결과: 진단 전용 Performance Schema overlay와 Hikari acquire timer를 같은 Checkout run에 결합하고, 성공·실패 모두 기본 Compose 설정으로 복구하도록 구성
- 검증: collector 66·timing aggregate 5 assertions, 2,000석 local mock fixture 50/75/100 RPS 측정, 복구 후 `performance_schema=OFF`·MariaDB health 확인, Backend CI 및 Reviewer `MERGE_READY: YES`
- 근거: [Checkout pool·DB 지연 상관 진단](docs/project-improvement/archive/checkout/checkout-pool-db-timing-correlation.md)
...
- PR: [#131](https://github.com/SKUWooU/TicketOnBoarding_Be/pull/131) / squash `d68ffd3`
- 결과: `active_seat_id`의 named unique constraint 하나만 유지하고 generated schema contract로 고정
- 검증: MariaDB `SHOW INDEX` contract·Checkout Testcontainers 53건·CI 성공
- 제외: 기존 DB migration, 실제 PG, 성능 수치 변경, Frontend
...
- PR: [#127](https://github.com/SKUWooU/TicketOnBoarding_Be/pull/127) / squash `802a1de`
- 결과: assignment active unique index의 supremum insert-intention deadlock을 Testcontainers barrier·local diagnostic으로 재현·기록
- 검증: Testcontainers 3회 deadlock/rollback·PowerShell 11 assertions·k6 inspect·CI 성공
- 제외: index/lock/pool 변경, 실제 PG·운영 성능 주장, Frontend
...
- PR: [#123](https://github.com/SKUWooU/TicketOnBoarding_Be/pull/123) / squash `552f066`
- 결과: local mock PG·2,000석 fixture의 k6 결과·snapshot·commit transition delta 교차 gate 구성
- 검증: parser/gate 6 assertions, k6 inspect·hot-seat 20 RPS·10초 smoke 통과; CI 재실행 240 tests 통과
